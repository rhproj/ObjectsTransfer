...
Переход на открытые решения в частности для баз данных поможет проекту снизить его себестоимость, сохранить бизнесу деньги которые могли бы быть потрачены на другие полезные вещи, например масштабирование (покупка того же железа).
...
По-этому, лучшим способом миграции структуры баз, является переключение провайдера с   на   и пересоздание миграционных файлов Entity Framework.
...
```
Создавая таким образом, столкнулся с трудностями мапинга колонок,  они должны называться так-же как в MS SQL, по этому на получение данныз из следующих колонок возвращалась ошибка
```sql
select 
logo_url 
--established_year
...
CREATE FOREIGN TABLE mssql_import.categories (
    ...
    "Name" text,
    "IsActive" boolean,
    "CreatedAt" text,  --создаю в текстовом виде
    "UpdatedAt" text
) SERVER mssql_server
OPTIONS (
    schema_name 'dbo',
...
```
Итого создано 12 foreign таблиц для базы **Product**
...
Перенлшу данные из foreign-table внешних таблиц в целевые:
###### brands
```sql
...
Т.е что сделал - загрузили поля, требующие приведения как есть в текстовом виде во временные таблицы, а потом их перенес из временных таблиц приводя к нужному типу в наши таблицы.  
